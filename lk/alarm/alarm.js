...
async function getRings() {
  const res = await getData('/ring?visible=true', 'get', {})
  if (res.success) {
    await setData(res.data)
  } else {
    fastMessage(res.message || 'Ошибка на сервере')
  }
}
...
async function setData(data) {
  ...
  for (i in data) {
    ...
    const ring_el = `<div class="alarm${class_active}" id="${id}"><div class="alarm_container" onclick="onClickAlarm(${id})")"><div class="circle"></div><div class="time">${time_str}</div><div class="remains"><p>До будильника</p><p>${time_left}</p></div></div></div>`
    container.insertAdjacentHTML('beforeend', ring_el)
    listenLongClick(document.getElementById(id), ()=>{goTo(`./alarm/edit/index.html?id=${id}`)}, 300)
  }
  if (data.length > 4) {
